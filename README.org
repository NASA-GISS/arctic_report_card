...
print("THROUGH: ", ds['time'].values[-1], "\n\n")

ds.sum(dim='region').\
      sel({'time':slice('2023-01-01','2023-12-31')})\
      .sum()[['MB','MB_err','SMB','SMB_err']]
#+END_SRC

#+RESULTS:
:RESULTS:
: THROUGH:  2023-09-15T00:00:00.000000000 
: 
: <xarray.Dataset>
: Dimensions:  ()
: Data variables:
...
ds.sum(dim='region')\
  ...
  .sum()[['MB','MB_err','SMB','SMB_err','D','D_err']]
#+END_SRC
...
: Data variables:
:     MB       float32 -201.5
:     MB_err   float32 94.29
:     SMB      float32 317.1
:     SMB_err  float32 28.54
:     D        float32 491.5
:     D_err    float32 46.41
